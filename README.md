...
Only the person setting up deployment needs the gem on their laptop. Production servers need **bash and git only** — no Ruby on the server.

- **CLI:** `git deploy <command>` (unchanged from upstream)
...
**Breaking change in v0.9.0:** `-r` is required on all remote commands (no default to `origin`).
